<template>
  <div class="min-h-screen bg-white">
        ...
        <div class="flex justify-between items-center">
          <div class="flex items-center gap-3">
            <router-link to="/" class="flex items-center gap-3 group">
              <div class="w-10 h-10 bg-gradient-to-br from-emerald-400 to-teal-500 rounded-lg flex items-center justify-center group-hover:scale-105 transition">
    ...
    <div class="border-b border-gray-100 bg-gradient-to-b from-gray-50 to-white">
      <div class="container mx-auto px-6 py-16">
        ...
        <p class="text-xl text-gray-600 max-w-2xl">
          Browse active loan requests and support students on their educational journey
        </p>
      </div>
    </div>
    ...
    <div class="container mx-auto px-6 py-12">
      <!-- Loading State -->
      <div v-if="loanStore.loading" class="flex flex-col items-center justify-center py-20">
      ...
      <!-- Empty State -->
      <div v-else-if="loanStore.loans.length === 0" class="flex flex-col items-center justify-center py-20">
      ...
      <div v-else>
        ...
        <div class="grid grid-cols-4 gap-6 mb-12">
          <div class="p-6 rounded-2xl border border-gray-100 bg-gradient-to-br from-white to-gray-50">
            ...
            <div class="text-3xl font-bold text-gray-900">{{ loanStore.loans.length }}</div>
          </div>
          <div class="p-6 rounded-2xl border border-gray-100 bg-gradient-to-br from-white to-gray-50">
            <div class="text-sm text-gray-500 mb-1 uppercase tracking-wide">Active</div>
            <div class="text-3xl font-bold text-emerald-500">{{ activeLoans }}</div>
          </div>
          <div class="p-6 rounded-2xl border border-gray-100 bg-gradient-to-br from-white to-gray-50">
            <div class="text-sm text-gray-500 mb-1 uppercase tracking-wide">Funded</div>
            <div class="text-3xl font-bold text-gray-900">{{ fundedLoans }}</div>
          </div>
          <div class="p-6 rounded-2xl border border-gray-100 bg-gradient-to-br from-white to-gray-50">
            <div class="text-sm text-gray-500 mb-1 uppercase tracking-wide">Total Volume</div>
            <div class="text-3xl font-bold text-gray-900">${{ totalVolume }}</div>
          </div>
        </div>
        ...
        <div class="grid md:grid-cols-3 gap-6">
          ...
          >
            ...
            <div class="flex justify-between items-start mb-4">
              <div>
                <h3 class="text-xl font-bold text-gray-900 mb-1">{{ loan.student_name }}</h3>
            ...
            <div class="space-y-3 mb-6">
              ...
              <div class="flex justify-between items-center">
                ...
                <span class="font-semibold text-gray-900">{{ loan.duration_months }}mo</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-gray-500 text-sm">Interest Rate</span>
                <span class="font-semibold text-emerald-600">{{ loan.interest_rate }}%</span>
              </div>
            </div>
            ...
            <div class="mb-4">
              ...
              <div class="flex justify-between text-xs mt-1">
                <span class="text-gray-400">${{ loan.amount_raised || 0 }} raised</span>
            ...
            <div v-if="loan.token_id" class="p-3 bg-gray-50 rounded-xl mb-4">
              <div class="flex items-center justify-between">
                ...
                <span class="text-xs font-mono text-gray-700">{{ loan.token_id.substring(0, 12) }}...</span>
              </div>
            </div>
        ...
        <div v-if="filteredLoans.length === 0" class="text-center py-12">
          <p class="text-gray-600">No loans found with status: <span class="font-semibold">{{ filter }}</span></p>
        </div>
      </div>
    </div>

    <!-- Footer CTA -->
    <div class="border-t border-gray-100 bg-gradient-to-br from-gray-50 to-white">
      <div class="container mx-auto px-6 py-16 text-center">
        <h2 class="text-3xl font-bold text-gray-900 mb-4">Want to create your own loan?</h2>
        <p class="text-gray-600 mb-8">Start your educational journey with MintEdu today</p>
        <router-link
          to="/"
...
const activeLoans = computed(() => {
  return loanStore.loans.filter(l => l.status === 'open' || l.status === 'funding').length
})
...
const totalVolume = computed(() => {
  return loanStore.loans.reduce((sum, loan) => sum + parseFloat(loan.amount), 0).toFixed(0)
})
