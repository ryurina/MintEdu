<template>
  <div class="min-h-screen bg-white">
    ...
    <div class="container mx-auto px-6 py-12">
      ...
      <div class="mb-12">
        ...
        <div class="grid md:grid-cols-4 gap-6">
          <div class="p-6 rounded-2xl border border-gray-100 bg-gradient-to-br from-white to-gray-50">
            <div class="text-sm text-gray-500 mb-1 uppercase tracking-wide">Total Loans</div>
            <div class="text-3xl font-bold text-gray-900">{{ myLoans.length }}</div>
          </div>
          <div class="p-6 rounded-2xl border border-gray-100 bg-gradient-to-br from-white to-gray-50">
            <div class="text-sm text-gray-500 mb-1 uppercase tracking-wide">Active</div>
            <div class="text-3xl font-bold text-emerald-500">{{ activeLoanCount }}</div>
          </div>
          <div class="p-6 rounded-2xl border border-gray-100 bg-gradient-to-br from-white to-gray-50">
            <div class="text-sm text-gray-500 mb-1 uppercase tracking-wide">Total Borrowed</div>
            <div class="text-3xl font-bold text-gray-900">${{ totalBorrowed }}</div>
          </div>
          <div class="p-6 rounded-2xl border border-gray-100 bg-gradient-to-br from-white to-gray-50">
            <div class="text-sm text-gray-500 mb-1 uppercase tracking-wide">Total Raised</div>
            <div class="text-3xl font-bold text-emerald-500">${{ totalRaised }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="mb-12">
        <div class="p-8 rounded-3xl border-2 border-gray-100 bg-white hover:border-emerald-200 transition">
          <div class="flex items-center gap-3 mb-8">
            ...
            <div>
              <h3 class="text-2xl font-bold text-gray-900">Create New Loan Request</h3>
              <p class="text-gray-500">Fill in the details to apply for funding</p>
            </div>
          </div>

          <div class="grid md:grid-cols-2 gap-6 mb-8">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">Loan Amount (USD)</label>
              <div class="relative">
          ...
          <!-- Preview Card -->
          <div v-if="loanForm.amount && loanForm.duration_months" class="p-6 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl mb-6 border border-emerald-100">
            <div class="text-sm text-emerald-700 font-semibold mb-3 uppercase tracking-wide">Preview</div>
            <div class="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span class="text-gray-600">Monthly Payment:</span>
                <span class="font-bold text-gray-900 ml-2">${{ monthlyPayment }}</span>
              </div>
              <div>
                <span class="text-gray-600">Total Repayment:</span>
                <span class="font-bold text-gray-900 ml-2">${{ totalRepayment }}</span>
              </div>
            </div>
          </div>
      ...
      <!-- My Loans List -->
      <div>
        <h3 class="text-2xl font-bold text-gray-900 mb-6">Your Loan Requests</h3>
        
        <!-- Empty State -->
        <div v-if="myLoans.length === 0" class="flex flex-col items-center justify-center py-16 px-4 rounded-3xl border-2 border-dashed border-gray-200">
          ...
          <p class="text-gray-600">Create your first loan request using the form above</p>
        </div>
        ...
        <div v-else class="grid md:grid-cols-2 gap-6">
          ...
          >
            ...
            <div class="flex justify-between items-start mb-6">
              <div>
                <h4 class="text-xl font-bold text-gray-900 mb-1">{{ loan.university }}</h4>
                <p class="text-gray-500 text-sm">Created {{ formatDate(loan.created_at) }}</p>
              </div>
              <span 
            ...
            <div class="mb-6">
              <div class="flex items-baseline gap-2 mb-2">
                <span class="text-4xl font-bold text-gray-900">${{ loan.amount }}</span>
            ...
            <div v-if="loan.token_id" class="p-4 bg-gray-50 rounded-xl mb-4">
              <div class="flex items-center justify-between mb-2">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs text-gray-500 font-medium uppercase tracking-wide">HTS Token</span>
                </div>
                <a 
              ...
              <span class="text-xs font-mono text-gray-700">{{ loan.token_id }}</span>
            </div>

            <!-- Actions -->
            <div class="flex gap-3">
              <button class="flex-1 py-3 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 transition font-medium text-sm">
                View Details
              </button>
              <button class="px-4 py-3 border border-gray-200 rounded-xl hover:bg-gray-50 transition">
...
const activeLoanCount = computed(() => {
  return myLoans.value.filter(l => l.status === 'open' || l.status === 'funding').length
})

const totalBorrowed = computed(() => {
  return myLoans.value.reduce((sum, loan) => sum + parseFloat(loan.amount), 0).toFixed(0)
})

const totalRaised = computed(() => {
  return myLoans.value.reduce((sum, loan) => sum + parseFloat(loan.amount_raised || 0), 0).toFixed(0)
})
